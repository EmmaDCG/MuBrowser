package com.mu.game.model.activity.imp.kfhd.qh;
...
public class QiangHuaElement extends ActivityElement {
   private HashMap itemMap = new HashMap();
   private HashMap unitMap = new HashMap();

   public QiangHuaElement(int id, QiangHuaActivity father) {
      super(id, father);
   }
   ...
   public boolean canReceive(Player player, boolean notice) {
      if (this.receiveOverload(player)) {
         ...
         return player.getEquipment().getTotalStar() < this.getNumerical() ? false : super.canReceive(player, notice);
      }
   }
   ...
   public int getReceiveStatus(Player player) {
      if (this.receiveOverload(player)) {
         ...
         return player.getEquipment().getTotalStar() < this.getNumerical() ? 0 : 1;
      }
   }
}
