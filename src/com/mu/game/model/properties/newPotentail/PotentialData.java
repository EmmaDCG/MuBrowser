package com.mu.game.model.properties.newPotentail;
...
public class PotentialData {
   ...
   private static HashMap professionOrder = new HashMap();
   private int professionType;
   private int level;
   private HashMap potentials = new HashMap();
   private int all = 0;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public PotentialData(int professionType, int level, HashMap potentials) {
      this.professionType = professionType;
      this.level = level;
      this.potentials = potentials;
   }

   public static int getKey(int professionType, int level) {
      return (professionType + 1) * 1000000 + level;
   }

   public static void initOrder(Sheet sheet) throws Exception {
      int rows = sheet.getRows();

      for(int i = 2; i <= rows; ++i) {
         int professionType = Tools.getCellIntValue(sheet.getCell("A" + i));
         String orderStr = Tools.getCellValue(sheet.getCell("B" + i));
         ...
         for(int var8 = 0; var8 < var9; ++var8) {
            String ss = var10[var8];
            StatEnum stat = StatEnum.find(Integer.parseInt(ss));
            switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[stat.ordinal()]) {
            case 2:
            case 3:
            case 4:
            case 5:
               if (orderList.contains(stat)) {
                  throw new Exception(sheet.getName() + " - 属性推荐顺序  属性ID重复填写 ，第  " + i + " 行");
               }

               orderList.add(stat);
               break;
            default:
               throw new Exception(sheet.getName() + " - 属性推荐顺序 属性ID填写错误 ，第 " + i + " 行");
            }
         }

         if (orderList.size() != 4) {
            throw new Exception(sheet.getName() + " - 属性推荐顺序 ，属性ID个数不对，第 " + i + " 行");
         }

         professionOrder.put(professionType, orderList);
      }
   ...
   public static void init(Sheet sheet) throws Exception {
      ...
      for(int i = 2; i <= rows; ++i) {
         ...
         int str = Tools.getCellIntValue(sheet.getCell("C" + i));
         int dex = Tools.getCellIntValue(sheet.getCell("D" + i));
         int intell = Tools.getCellIntValue(sheet.getCell("E" + i));
         int con = Tools.getCellIntValue(sheet.getCell("F" + i));
         if (str < 0 || dex < 0 || intell < 0 || con < 0) {
            throw new Exception(sheet.getName() + "-推荐属性点数据模板有误 ，第 " + i + " 行");
         }

         int all = str + dex + intell + con;
         if (all < 1) {
            throw new Exception(sheet.getName() + " -推荐属性点数据模板总数据有误 ，第 " + i + " 行");
         }

         HashMap potentials = new HashMap();
         potentials.put(StatEnum.STR, str);
         potentials.put(StatEnum.DEX, dex);
         potentials.put(StatEnum.INT, intell);
         potentials.put(StatEnum.CON, con);
         PotentialData data = new PotentialData(professionType, level, potentials);
         data.setAll(all);
         dataMap.put(getKey(professionType, level), data);
      }

   }

   public static void initCheck() throws Exception {
      int[] proTypes = Profession.proTypes;
      int[] var4 = proTypes;
      int var3 = proTypes.length;

      for(int var2 = 0; var2 < var3; ++var2) {
         int proType = var4[var2];
         if (!professionOrder.containsKey(proType)) {
            throw new Exception("属性推荐顺序  - " + proType + " 职业的数据不存在");
         }

         for(int i = 1; i <= VariableConstant.Max_Level_2Exp; ++i) {
            int key = getKey(proType, i);
            if (!dataMap.containsKey(key)) {
               throw new Exception("属性推荐 - 数据不完整 (" + proType + "," + i + ")");
            }
         }
      }
   ...
   public static PotentialData getData(int proType, int level) {
      int key = getKey(proType, level);
      return (PotentialData)dataMap.get(key);
   }

   public static List getProTypeOrder(int proType) {
      return (List)professionOrder.get(proType);
   }

   public static HashMap allocat(Player player, int allPoten) {
      if (allPoten <= 0) {
         return null;
      } else {
         PotentialData data = getData(player.getProType(), player.getLevel());
         HashMap map = new HashMap();
         List statOrder = getProTypeOrder(player.getProType());
         int targetValue = 0;
         boolean last = false;

         for(int i = 0; i < statOrder.size(); ++i) {
            StatEnum stat = (StatEnum)statOrder.get(i);
            last = i == statOrder.size() - 1;
            if (last) {
               map.put(stat, allPoten);
               break;
            }

            int curValue = player.getStatValue(stat);
            int needVaule = data.getNeedStatValue(stat);
            if (needVaule > curValue) {
               targetValue = Math.min(allPoten, needVaule - curValue);
               map.put(stat, targetValue);
               allPoten -= targetValue;
               if (allPoten <= 0) {
                  break;
               }
            }
   ...
   public int getNeedStatValue(StatEnum stat) {
      return this.potentials.containsKey(stat) ? ((Integer)this.potentials.get(stat)).intValue() : 0;
   }
   ...
   public HashMap getPotentials() {
      return this.potentials;
   }

   public void setPotentials(HashMap potentials) {
      this.potentials = potentials;
   }
