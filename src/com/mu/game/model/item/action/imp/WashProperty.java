...
public class WashProperty extends ItemAction {
   private StatEnum stat = null;
   private int washValue = 1;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public WashProperty(StatEnum stat, int washValue) {
      this.stat = stat;
      this.washValue = washValue;
   }

   protected int doAction(Player player, Item item, int useNum, boolean definite, int wantedSlot) {
      int result = AddBasicProperty.canSetPotential(player);
      if (result != 1) {
         return result;
      } else {
         int curPotential = this.getCurrentCanWashPotential(player, this.stat);
         if (curPotential < 1) {
            return 1034;
         } else {
            int str = this.stat == StatEnum.STR ? -curPotential : 0;
            int intell = this.stat == StatEnum.INT ? -curPotential : 0;
            int dex = this.stat == StatEnum.DEX ? -curPotential : 0;
            int con = this.stat == StatEnum.CON ? -curPotential : 0;
            result = player.getItemManager().deleteItem(item, 1, 20).getResult();
            if (result == 1) {
               PlayerManager.changeBasicProperty(player, str, dex, con, intell, true);
            }
   ...
   private int getCurrentCanWashPotential(Player player, StatEnum stat) {
      switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[stat.ordinal()]) {
      case 2:
         return Math.min(player.getPoSTR(), this.washValue);
      case 3:
         return Math.min(player.getPoDEX(), this.washValue);
      case 4:
         return Math.min(player.getPoINT(), this.washValue);
      default:
         return Math.min(player.getPoCON(), this.washValue);
      }
   }
   ...
   public void initCheck(String des) throws Exception {
      ...
      case 4:
      case 5:
         if (this.washValue >= 1 && this.washValue <= 100) {
            return;
         }

         throw new Exception(des + "- 洗点数值不正确。需要大于0 小于等于" + 100);
      default:
         throw new Exception(des + " - 洗点，属性填写错误");
      }
   }
