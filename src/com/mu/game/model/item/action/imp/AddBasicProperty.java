...
public class AddBasicProperty extends ItemAction {
   private HashMap basicProMap = null;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public AddBasicProperty(HashMap basicProMap) {
      this.basicProMap = basicProMap;
   }

   protected int doAction(Player player, Item item, int useNum, boolean definite, int wantedSlot) {
      int result = canSetPotential(player);
      if (result != 1) {
         return result;
      } else {
         HashMap addMap = new HashMap();
         this.getStatValue(player, addMap);
         if (addMap.size() < 1) {
            addMap.clear();
            addMap = null;
            return 3035;
         } else {
            int str = addMap.containsKey(StatEnum.STR) ? ((Integer)addMap.get(StatEnum.STR)).intValue() : 0;
            int dex = addMap.containsKey(StatEnum.DEX) ? ((Integer)addMap.get(StatEnum.DEX)).intValue() : 0;
            int con = addMap.containsKey(StatEnum.CON) ? ((Integer)addMap.get(StatEnum.CON)).intValue() : 0;
            int intell = addMap.containsKey(StatEnum.INT) ? ((Integer)addMap.get(StatEnum.INT)).intValue() : 0;
            result = player.getItemManager().deleteItem(item, 1, 20).getResult();
            if (result == 1) {
               PlayerManager.changeBasicProperty(player, str, dex, con, intell, false);
            }
   ...
   private void getStatValue(Player player, HashMap addMap) {
      Iterator var4 = this.basicProMap.values().iterator();

      while(var4.hasNext()) {
         AddNumber number = (AddNumber)var4.next();
         int value = this.getRoleFruitValue(player, number.getStat());
         value = 500 - value;
         value = Math.min(value, number.getRndValue());
         if (value > 0) {
            addMap.put(number.getStat(), value);
         }
      }

   }

   private int getRoleFruitValue(Player player, StatEnum stat) {
      PlayerLevelData levelData = PlayerLevelData.getLevelData(player.getProType(), player.getLevel());
      int value = player.getProperty().getLoadedStat(stat);
      switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[stat.ordinal()]) {
      case 2:
         value -= levelData.getStr() + player.getPoSTR();
         break;
      case 3:
         value -= levelData.getDex() + player.getPoDEX();
         break;
      case 4:
         value -= levelData.getIntell() + player.getPoINT();
         break;
      case 5:
         value -= levelData.getCon() + player.getPoCON();
      }
   ...
   public static int canSetPotential(Player player) {
      long now = System.currentTimeMillis();
      long last = player.getLastSetPotential();
      return now - last < 200L ? 1032 : 1;
   }

   public void initCheck(String des) throws Exception {
      if (this.basicProMap != null && this.basicProMap.size() >= 1) {
         Iterator var3 = this.basicProMap.values().iterator();

         while(var3.hasNext()) {
            AddNumber numberStat = (AddNumber)var3.next();
            switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[numberStat.getStat().ordinal()]) {
            case 2:
            case 3:
            case 4:
            case 5:
               if (numberStat.getMinCount() < 1 || numberStat.getMaxCount() < 1) {
                  throw new Exception(des + " : 果实 数值填写错误");
               }

               if (numberStat.getMinCount() > numberStat.getMaxCount()) {
                  int tmpValue = numberStat.getMinCount();
                  numberStat.setMinCount(numberStat.getMaxCount());
                  numberStat.setMaxCount(tmpValue);
               }
               break;
            default:
               throw new Exception(des + " : 果实 数据属性ID填写错误");
            }
         }

      } else {
         throw new Exception(des + " : 果实数据没有填写");
      }
   }

   public boolean obtainFunctionTip(Player player, ItemModel model) {
      boolean canEat = false;
      SortedMap needStatMap = model.getNeedBasicPro();
      Iterator var6 = needStatMap.keySet().iterator();
      ...
      while(var6.hasNext()) {
         ...
         case 4:
         case 5:
            int value = this.getRoleFruitValue(player, stat);
            if (value < 500) {
               canEat = true;
            }
         }
      ...
      if (canEat && super.obtainFunctionTip(player, model)) {
         return true;
      } else {
         return false;
      }
