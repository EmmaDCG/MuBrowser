...
public abstract class ItemAction {
   ...
   public int action(Player player, Item item, int useNum, boolean definite, int wantedSlot) {
      int result = this.canUse(player, item, useNum, definite);
      return result != 1 ? result : this.doAction(player, item, useNum, definite, wantedSlot);
   }

   protected abstract int doAction(Player var1, Item var2, int var3, boolean var4, int var5);

   public abstract int privyCondition(Player var1, Item var2, int var3, boolean var4);

   public abstract void useWhenObtaining(Player var1, ItemModel var2, int var3, boolean var4);
   ...
   public int canUse(Player player, Item item, int useNum, boolean definite) {
      int result = this.commonCondition(player, item, useNum, definite);
      return result != 1 ? result : this.privyCondition(player, item, useNum, definite);
   }
   ...
   protected int commonCondition(Player player, Item item, int useNum, boolean definite) {
      if (item != null && item.getModel() != null && item.getCount() >= 1) {
         if (item.getModel().getAction() == null) {
            return 3007;
         } else if (this.isInCoolTime(player, item)) {
            return 3008;
         } else if (TransactionManager.isTransaction(item.getID())) {
            return 3005;
         } else if (item.isTimeExpired(System.currentTimeMillis())) {
            return 3044;
         } else if (item.getModel().getUseType() == 2) {
            return 3003;
         } else {
            int result = canUse(item.getModel(), player);
            if (result != 1) {
               return result;
            } else {
               int remainCount = this.getRemainUseCount(player, item.getModelID());
               if (remainCount <= 0) {
                  return 3020;
               } else if (useNum < 1) {
                  return 8;
               } else {
                  return useNum > 1 && !ItemModel.getModel(item.getModelID()).isCanBatch() ? 3007 : 1;
               }
            }
         }
      } else {
   ...
   public static int canUse(ItemModel model, Player player) {
      if (model.getUseLevel() > player.getLevel()) {
         return 3004;
      } else if (model.getProfession().size() > 0 && !model.getProfession().contains(player.getProfessionID())) {
         return 3006;
      } else {
         if (model.getNeedBasicPro().size() > 0) {
            Iterator it = model.getNeedBasicPro().entrySet().iterator();

            while(it.hasNext()) {
               Entry entry = (Entry)it.next();
               if (player.getStatValue((StatEnum)entry.getKey()) < ((Integer)entry.getValue()).intValue()) {
                  StatEnum stat = (StatEnum)entry.getKey();
   ...
   protected static int calCanUseCount(int maxUseCount, int itemCount, int everyValue, int maxValueCanDo) {
      if (everyValue < 1) {
         return 0;
      } else {
         int count = Math.max(1, Math.min(maxUseCount, itemCount));
         long maxValue = (long)Math.min(maxValueCanDo, everyValue * count);
         count = Math.max(1, (int)(maxValue / (long)everyValue));
         return count;
      }
   }

   private boolean isInCoolTime(Player player, Item item) {
      return !player.getItemManager().canUseItem(item);
   }

   public int getRemainUseCount(Player player, int modelID) {
      int count = player.getItemManager().getRemainUseCount(modelID);
      return count;
   }
