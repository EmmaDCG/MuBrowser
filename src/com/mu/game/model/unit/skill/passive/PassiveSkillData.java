package com.mu.game.model.unit.skill.passive;
...
public class PassiveSkillData {
   ...
   private int skillID;
   private String des;
   private StatEnum statID;
   private int shortcutPanelID = -1;
   // $FF: synthetic field
   ...
   public static void init(Sheet sheet) throws Exception {
      ...
      while(i <= rows) {
         ...
         int stat = Tools.getCellIntValue(sheet.getCell("C" + i));
         int shortcutPanelID = Tools.getCellIntValue(sheet.getCell("D" + i));
         StatEnum statID = StatEnum.find(stat);
         switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[statID.ordinal()]) {
         case 112:
         case 113:
         case 114:
         case 115:
         case 116:
            ...
            data.setShortcutPanelID(shortcutPanelID);
            dataMap.put(skillID, data);
            ++i;
            break;
         default:
            throw new Exception(sheet.getName() + "-属性ID不正确，第 " + i + " 行");
         }
      }

   }

   public static boolean hasData(int skillID) {
      return dataMap.containsKey(skillID);
   }

   public static PassiveSkillData getData(int skillID) {
      return (PassiveSkillData)dataMap.get(skillID);
   }

   public void checkInit(String des) throws Exception {
      if (!ShortcutBuyPanel.hasShortcutBuy(this.shortcutPanelID)) {
         throw new Exception(des + " - 快速购买并使用数据不存在");
      }
   }
   ...
   public StatEnum getStatID() {
      return this.statID;
   }

   public void setStatID(StatEnum statID) {
      this.statID = statID;
   }

   public int getShortcutPanelID() {
      return this.shortcutPanelID;
   }

   public void setShortcutPanelID(int shortcutPanelID) {
      this.shortcutPanelID = shortcutPanelID;
   }
