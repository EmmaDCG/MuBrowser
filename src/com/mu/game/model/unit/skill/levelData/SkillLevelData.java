...
public class SkillLevelData {
   ...
   private Item needItem = null;
   private float skillCorrection;
   ...
   public static void addLevelData(SkillLevelData levelData) {
      HashMap datas = (HashMap)levelDatas.get(levelData.getSkillID());
      if (datas == null) {
         datas = new HashMap();
         levelDatas.put(levelData.getSkillID(), datas);
      }

      datas.put(levelData.getLevel(), levelData);
   }

   public static SkillLevelData getLevelData(int skillID, int level) {
      return !levelDatas.containsKey(skillID) ? null : (SkillLevelData)((HashMap)levelDatas.get(skillID)).get(level);
   }

   public int getStatVlaue(StatEnum stat, StatModifyPriority priority) {
      if (this.stats != null) {
         Iterator var4 = this.stats.iterator();

         while(var4.hasNext()) {
            FinalModify modify = (FinalModify)var4.next();
            if (modify.getStat() == stat && modify.getPriority() == priority) {
   ...
   public int getAGConsume() {
      return this.AGConsume;
   }

   public void setAGConsume(int aGConsume) {
      this.AGConsume = aGConsume;
   }
   ...
   public void setNeedItem(Item needItem) {
