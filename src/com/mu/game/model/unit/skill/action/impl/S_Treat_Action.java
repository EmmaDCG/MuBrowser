...
public class S_Treat_Action extends SkillAction {
   private int value;
   private boolean percent;
   private StatEnum stat;
   private int statAddition;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public S_Treat_Action(int type, int value, boolean percent, StatEnum stat, int statAddition) {
      super(type);
      this.value = value;
   ...
   public int preCheck(Skill skill, Creature effected, Point specifyPoint) {
      if (effected.isDie()) {
         ...
         return effected.getHp() >= effected.getMaxHp() ? 1009 : 1;
      }
   }

   public void singleAction(SingleSkill skill, Creature effected, AttackResult result, Point centPoint) {
      int aValue = this.getActualValue(effected, skill.getOwner());
      AddHp.addHPAndSend(effected, aValue);
   }

   public void groupAction(GroupSkill skill, HashMap results, Point centPoint) {
      Iterator var5 = results.keySet().iterator();

      while(var5.hasNext()) {
         Creature effected = (Creature)var5.next();
         int aValue = this.getActualValue(effected, skill.getOwner());
         AddHp.addHPAndSend(effected, aValue);
      }
   ...
   public int getActualValue(Creature effected, Creature owner) {
      int aValue = this.value;
      int aStatAddition = owner.getStatValue(this.stat) * this.statAddition;
      if (this.percent) {
         aValue = Constant.getPercentValue(effected.getMaxHp(), aValue);
      }

      aValue += aStatAddition;
      aValue = Math.min(aValue, effected.getMaxHp() - effected.getHp());
      aValue = Math.max(1, aValue);
      return aValue;
   }
   ...
   public Creature useTargetInHang(Player owner, boolean assistedMode) {
      ...
      if (assistedMode && team != null) {
         ...
         while(var7.hasNext()) {
            ...
            if (p != null && p.getMap().equals(owner.getMap()) && AttackCreature.distanceCheck(p.getPosition(), owner.getPosition(), 5000) == 1) {
               int tmpHpPer = this.getHpPercent(p);
               if (this.needToTreat(tmpHpPer) && hpPercent > tmpHpPer) {
                  hpPercent = tmpHpPer;
                  target = p;
               }
            }
         ...
         return this.needToTreat(this.getHpPercent(owner)) ? owner : null;
      }
   }

   private boolean needToTreat(int hpPercent) {
      return hpPercent <= 70000;
   }

   private int getHpPercent(Player player) {
      int hpPercent = Constant.getPercent(player.getHp(), player.getMaxHp());
      return hpPercent;
   }

   public void initCheck(String des) throws Exception {
      if (this.value < 1) {
         throw new Exception(des + "-治疗-血量填写不正确");
      } else {
         if (this.stat != StatEnum.None) {
            switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[this.stat.ordinal()]) {
            case 2:
            case 3:
            case 4:
            case 5:
               if (this.statAddition < 1) {
                  throw new Exception(des + " - 治疗 - 属性提升数值不合理");
               }
               break;
            default:
               throw new Exception(des + "-治疗-提升属性ID填写不正确");
            }
         }
