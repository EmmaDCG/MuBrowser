...
public class BuffCapacity extends Buff {
   protected static final int INTERVAL = 20000;
   private HashMap shuxing = new HashMap();
   private int intervalTime = 0;
   private boolean change = false;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public BuffCapacity(int modelID, int level, Creature owner, Creature castor) {
      super(modelID, level, owner, castor);
   }

   public void startDetail() {
      if (this.getProps() != null && this.getProps().size() >= 0) {
         this.intervalTime = this.getModel().getIntervalTime();
         if (this.intervalTime < 20000) {
            this.intervalTime = 20000;
         }

         this.setBuffCheckTime(this.intervalTime);
         this.checkCanAdd();
         this.capacityImpl();
      } else {
         this.end(false);
   ...
   public void capacityImpl() {
      this.change = false;
      if (!this.getOwner().isDie() && !this.getOwner().isDestroy()) {
         if (this.getOwner().getType() != 1 || !((Player)this.getOwner()).isNew()) {
            Iterator var2 = this.getProps().iterator();

            while(var2.hasNext()) {
               BuffProps prop = (BuffProps)var2.next();
               switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[prop.getStat().ordinal()]) {
               case 7:
                  this.changeHp(this.getOwner(), prop);
               case 8:
               case 9:
               case 10:
               default:
                  break;
               case 11:
                  this.changeMp(this.getOwner(), prop);
               }
            }

            if (this.change) {
               boolean end = true;
               Iterator var3 = this.getProps().iterator();
               ...
               if (this.getOwner().getType() == 1) {
                  BuffUpdateDydata.sendToClient((Player)this.getOwner(), this);
               }
   ...
   private void changeMp(Creature creature, BuffProps prop) {
      try {
         if (creature.getMp() < creature.getMaxMp()) {
            int tempValue = creature.getMaxMp() - creature.getMp();
            tempValue = Math.min(tempValue, prop.getValue());
            if (tempValue <= 0) {
               return;
            }

            creature.increaseMp(tempValue);
            CreatureHpMpPacket.creatureMpChange(creature);
            this.change = true;
            prop.setValue(prop.getValue() - tempValue);
         }
      } catch (Exception var4) {
         var4.printStackTrace();
      }
   ...
   private void changeHp(Creature creature, BuffProps prop) {
      try {
         if (creature.getHp() < creature.getMaxHp()) {
            int tempValue = creature.getMaxHp() - creature.getHp();
            tempValue = Math.min(tempValue, prop.getValue());
            if (tempValue <= 0) {
               return;
            }

            AddHp.addHPAndSend(creature, tempValue);
            this.change = true;
            prop.setValue(prop.getValue() - tempValue);
         }
      } catch (Exception var4) {
   ...
   private void checkCanAdd() {
      if (this.getProps() != null) {
         ...
         while(var4.hasNext()) {
            ...
            if (bp.getValue() > tempMaxValue) {
               tempMaxValue = bp.getValue();
            }

            int shuxingValue = this.getShuxingValue(bp.getStat());
            if (shuxingValue > 0) {
               int tto = 1;
               if (bp.getValue() % shuxingValue == 0) {
                  tto = bp.getValue() / shuxingValue;
               } else {
                  tto = bp.getValue() / shuxingValue + 1;
               }
         ...
         this.setOverlap(tmpOverlap);
      }

   }

   private int getShuxingValue(StatEnum stat) {
      return this.shuxing.containsKey(stat) ? ((Integer)this.shuxing.get(stat)).intValue() : 0;
   }

   public void doOverlapping() {
      Buff buff = this.getOwner().getBuffManager().getBuff(this.getModelID());
      if (buff != null) {
         List tempprops = null;
         if (buff.getProps() != null) {
            tempprops = new ArrayList();
            tempprops.addAll(buff.getProps());
         }

         buff.end(true);
         buff = null;
         if (tempprops != null) {
            Iterator var4 = tempprops.iterator();
   ...
   public void initBuff(long duration, List props, int createType) {
      super.initBuff(duration, props, createType);
      if (this.getProps() != null) {
         Iterator var6 = this.getProps().iterator();

         while(var6.hasNext()) {
            BuffProps bp = (BuffProps)var6.next();
            if (bp.getValue() > 0) {
               this.shuxing.put(bp.getStat(), bp.getValue());
            }
         }
      }
