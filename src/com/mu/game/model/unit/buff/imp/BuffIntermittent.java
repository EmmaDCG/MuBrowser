...
public class BuffIntermittent extends Buff {
   private int casterAtk = 1;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public BuffIntermittent(int modelID, int level, Creature owner, Creature caster) {
      super(modelID, level, owner, caster);
      this.casterAtk = Rnd.get(caster.getStatValue(StatEnum.ATK_MIN), caster.getStatValue(StatEnum.ATK_MAX));
      this.casterAtk = Math.max(1, this.casterAtk);
   }

   public void startDetail() {
      this.setBuffCheckTime(Math.max(500, this.getModel().getIntervalTime()));
      this.intermittent();
   }
   ...
   private void intermittent() {
      try {
         Creature caster = this.getCaster();
         if (this.isDestory() || caster == null) {
            return;
         }

         int type = 8;
         if (!this.getModel().isDebuff()) {
            type = 6;
         }

         if (this.getProps() != null && this.getProps().size() > 0) {
            Iterator it = this.getProps().iterator();

            while(it.hasNext()) {
               BuffProps prop = (BuffProps)it.next();
               StatEnum stat = prop.getStat();
               ...
               case 111:
                  if (stat == StatEnum.HP) {
                     if (prop.isPercent()) {
                        value = Constant.getPercentValue(this.getOwner().getMaxHp(), value);
                     }
                  } else {
                     value = Constant.getPercentValue(this.casterAtk, value);
                  }

                  value = Math.abs(value);
                  result = new AttackResult(type, value, 0, -1, (Creature)null);
                  result.setCasterID(this.getCasterType(), this.getCasterID());
                  if (this.getModel().isDebuff()) {
                     value = Math.min(value, this.getOwner().getHp() - 1);
                     result.setDamage(value);
                     AddHp.reduceHp(this.getOwner(), result, caster);
                  } else {
                     AddHp.addHPAndSend(this.getOwner(), value);
                  }
                  break;
               case 11:
                  if (prop.isPercent()) {
                     value = (int)((double)this.getOwner().getMaxMp() * 1.0D * (double)value / 100000.0D);
                  }

                  value = Math.abs(value);
                  if (this.getModel().isDebuff()) {
                     AddMp.reduceMPAndSend(this.getOwner(), value);
                  } else {
                     AddMp.addMPAndSend(this.getOwner(), value);
                  }
               }
            }
         }
