...
public class BuffWorldLevel extends BuffCommon {
   private int curWorldLevel;
   private int levelGap;
   private int expBonus;
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public BuffWorldLevel(int modelID, int level, Player owner, Creature caster) {
      super(modelID, level, owner, caster);
      this.setBuffCheckTime(1000);
   }

   protected void addProp() {
      if (this.getFartherEffect() != null) {
         Iterator var2 = this.getFartherEffect().iterator();

         while(var2.hasNext()) {
            FinalModify fm = (FinalModify)var2.next();
            switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[fm.getStat().ordinal()]) {
            case 40:
               this.expBonus = fm.getValue();
               break;
            case 60:
               this.curWorldLevel = fm.getValue();
               break;
            case 61:
               this.levelGap = fm.getValue();
            }
         }
      }
      ...
      if (this.expBonus > 0) {
         ...
         BuffProps bp = new BuffProps(StatEnum.EXP_BONUS, this.expBonus, this.expBonus, StatModifyPriority.ADD, 0);
         ps.add(bp);
         this.setProps(ps);
      }
   ...
   protected void checkEffect() {
      this.setHasEffect(this.expBonus > 0);
   }

   public List getDynamicData() {
      List dyDatas = new ArrayList();
      String s = MessageText.getText(1036);
      if (this.expBonus < 1) {
         s = MessageText.getText(1037).replace("%d%", "30");
      }

      s = s.replace("%s%", Tools.isMaster(this.curWorldLevel) ? MessageText.getText(1038) + Tools.getMasterLevel(this.curWorldLevel) : "" + this.curWorldLevel);
      s = s.replace("%m%", "" + this.levelGap);
      if (this.expBonus > 0) {
         s = s.replace("%n%", "" + this.expBonus / 1000);
      }
   ...
   public static List createParams(int worldLevel, int levelGap, int expBonus) {
      List fmList = new ArrayList();
      FinalModify wlModify = new FinalModify(StatEnum.WORLDLEVEL, worldLevel, StatModifyPriority.ADD);
      fmList.add(wlModify);
      FinalModify lgModify = new FinalModify(StatEnum.LEVELGAP, levelGap, StatModifyPriority.ADD);
      fmList.add(lgModify);
      FinalModify expModify = new FinalModify(StatEnum.EXP_BONUS, expBonus, StatModifyPriority.ADD);
      fmList.add(expModify);
      return fmList;
   }
