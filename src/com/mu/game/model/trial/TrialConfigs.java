...
public class TrialConfigs {
   private static HashMap trialMap = new HashMap();
   private static HashMap trialStrMap = new HashMap();
   private static HashMap rewardMap = new HashMap();
   private static TrialConfigs zeroConfig = null;
   private static TrialReward fullReward = null;
   private static int maxLevel = 0;
   private static int maxRewardTimes = 0;
   private static int basicReceiveTimes = 1;
   private int level = 0;
   private String name;
   private int buffId;
   ...
   private int icon;
   private String des;
   private int warCommentLevel;
   private int warCommentRank;
   private int energyReward = 1000;
   ...
   public static void initTrial(InputStream in) throws Exception {
      Workbook wb = Workbook.getWorkbook(in);
      initConfig(wb.getSheet(1));
      initReward(wb.getSheet(2));
      initOthers(wb.getSheet(3));
   }

   private static void initConfig(Sheet sheet) throws Exception {
      int rows = sheet.getRows();

      for(int i = 2; i <= rows; ++i) {
         TrialConfigs config = new TrialConfigs(Tools.getCellIntValue(sheet.getCell("A" + i)));
         config.setName(Tools.getCellValue(sheet.getCell("B" + i)));
         config.setIcon(Tools.getCellIntValue(sheet.getCell("C" + i)));
         config.setDes(Tools.getCellValue(sheet.getCell("D" + i)));
         config.setBuffId(Tools.getCellIntValue(sheet.getCell("E" + i)));
         config.setBuffLevel(Tools.getCellIntValue(sheet.getCell("F" + i)));
         config.setWarCommentLevel(Tools.getCellIntValue(sheet.getCell("G" + i)));
         config.setWarCommentRank(Tools.getCellIntValue(sheet.getCell("H" + i)));
         config.setEnergyReward(Tools.getCellIntValue(sheet.getCell("I" + i)));
         config.setRewardDes(Tools.getCellValue(sheet.getCell("J" + i)));
         config.setNameIcon(Tools.getCellIntValue(sheet.getCell("L" + i)));
         if (config.getLevel() != 0) {
            trialMap.put(config.getLevel(), config);
            trialStrMap.put(config.getWarCommentLevel() + "_" + config.getWarCommentRank(), config);
         } else {
            zeroConfig = config;
         }
         ...
         config.initReward();
         config.setSuccessDes(Tools.getCellValue(sheet.getCell("K" + i)));
      }

   }

   private static void initReward(Sheet sheet) throws Exception {
      int rows = sheet.getRows();

      for(int i = 2; i <= rows; ++i) {
         TrialReward reward = new TrialReward(Tools.getCellIntValue(sheet.getCell("A" + i)));
         reward.setConsumeType(Tools.getCellIntValue(sheet.getCell("B" + i)));
         reward.setConsume(Tools.getCellIntValue(sheet.getCell("C" + i)));
         reward.setDescription(Tools.getCellValue(sheet.getCell("D" + i)));
         if (reward.getTimes() != 0) {
            rewardMap.put(reward.getTimes(), reward);
         } else {
            fullReward = reward;
         }

         if (maxRewardTimes < reward.getTimes()) {
            maxRewardTimes = reward.getTimes();
         }
      }

   }

   private static void initOthers(Sheet sheet) throws Exception {
      basicReceiveTimes = Tools.getCellIntValue(sheet.getCell("B2"));
   }

   public static int getMaxRewardTimes() {
      return maxRewardTimes;
   }

   public static TrialReward getFullReward() {
      return fullReward;
   }

   public static TrialConfigs getZeroConfig() {
      return zeroConfig;
   }
   ...
   public TrialConfigs(int level) {
      this.level = level;
   }

   public static HashMap getTrialMap() {
      return trialMap;
   }

   public static String getTrialReceiveMessage(int level) {
      TrialConfigs config = getConfig(level);
      if (config == null) {
         config = zeroConfig;
      }

      TrialConfigs nextConfig = getConfig(level + 1);
      String msg = config.getRewardDes().replace("%s%", String.valueOf(config.getEnergyReward()));
      if (nextConfig != null) {
         msg = msg.replace("%n%", String.valueOf(nextConfig.getEnergyReward()));
      }

      return msg;
   }

   public static void setTrialMap(HashMap trialMap) {
      trialMap = trialMap;
   }

   public static TrialReward getReward(int times) {
      return (TrialReward)rewardMap.get(times);
   }

   public static int getBasicReceiveTimes() {
      return basicReceiveTimes;
   }

   public static int getPlayerTrialMaxReceiveTimes(Player player) {
      return Math.min(basicReceiveTimes, maxRewardTimes);
   }
   ...
   public void setSuccessDes(String successDes) {
      this.successDes = successDes.replace("%m%", String.valueOf(this.getMoneyRate())).replace("%e%", String.valueOf(this.getExpRate()));
   }

   public void initReward() {
      BuffDynamicData data = BuffDynamicData.getDyData(this.buffId, this.buffLevel);
      if (data != null) {
         List list = data.getProps();
         Iterator var4 = list.iterator();

         while(var4.hasNext()) {
            BuffProps pro = (BuffProps)var4.next();
            switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[pro.getStat().ordinal()]) {
            case 40:
               this.expRate = pro.getValue() / 1000;
               break;
            case 145:
               this.moneyRate = pro.getValue() / 1000;
            }
         }
   ...
   public int getWarCommentLevel() {
      return this.warCommentLevel;
   }

   public void setWarCommentLevel(int warCommentLevel) {
      this.warCommentLevel = warCommentLevel;
   }

   public int getWarCommentRank() {
      return this.warCommentRank;
   }

   public void setWarCommentRank(int warCommentRank) {
      this.warCommentRank = warCommentRank;
   }

   public static TrialConfigs getConfig(int level) {
      return (TrialConfigs)trialMap.get(level);
   }
