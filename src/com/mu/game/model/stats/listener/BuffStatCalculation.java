...
public class BuffStatCalculation {
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public static List addBuff(Creature owner, List props, int buffModelID) {
      List statList = null;
      List fmList = null;
      if (props != null) {
         FinalModify fm = null;
         ...
         while(var7.hasNext()) {
            BuffProps prop = (BuffProps)var7.next();
            ...
            case 10:
            default:
               fm = new FinalModify(prop.getStat(), prop.getValue(), prop.getPriority());
               if (fmList == null) {
      ...
      if (fmList != null) {
         StatChange.addStat(owner, StatIdCreator.createBuffId(buffModelID), fmList, StatChange.isSendStat(owner));
      }

      return (List)statList;
   }
