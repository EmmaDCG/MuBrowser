package com.mu.game.model.stats.listener;
...
public class EquipmentListener {
   ...
   public static List getItemFinalStat(Item item, float coeff, List ridingStatList) {
      List list = new ArrayList();
      ridingStatList.clear();
      WeaponType wt = ItemType.getWeaponType(item.getItemType());
      boolean zhuijiaCoeff = true;
      if (wt == WeaponType.SecondaryHand || wt == WeaponType.None) {
         zhuijiaCoeff = false;
      }

      int starLevel = item.getStarLevel();
      Iterator var8 = item.getBasisStats().iterator();

      while(true) {
         ItemModify bm;
         while(var8.hasNext()) {
            bm = (ItemModify)var8.next();
            int value = bm.getValue();
            if (starLevel > 0) {
               value += StarForgingData.getCurrentValue(item, bm.getStat());
            }

            if (zhuijiaCoeff) {
               StatEnum stat = bm.getStat();
               ...
               case 34:
               default:
                  value += ZhuijiaForgingData.getCurrentValue(item, stat);
               }
            }

            value = (int)((float)value * coeff);
            FinalModify fm = new FinalModify(bm.getStat(), value, bm.getPriority());
            if (item.getSlot() == 13 && bm.getStat() == StatEnum.SPEED) {
               ridingStatList.add(fm);
            } else {
               list.add(fm);
               switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[bm.getStat().ordinal()]) {
               case 35:
                  fm = new FinalModify(StatEnum.ATK_MIN, value, bm.getPriority());
                  list.add(fm);
                  break;
               case 36:
                  fm = new FinalModify(StatEnum.ATK_MAX, value, bm.getPriority());
                  list.add(fm);
               }
            }
         }
         ...
         if (!zhuijiaCoeff) {
            ZhuijiaForgingData.addZhjToFinalModify(item.getItemType(), item.getZhuijiaLevel(), list);
         }

         var8 = item.getOtherStats().values().iterator();

         while(var8.hasNext()) {
            bm = (ItemModify)var8.next();
            FinalModify fm;
            switch(bm.getBonusType()) {
            case 2:
               if (bm.getStat() == StatEnum.ATK_LUCKY_RATE) {
                  fm = bm.createFinalModify();
                  list.add(fm);
               }
               break;
            default:
         ...
         if (item.getModel().getStarActivation().size() > 0) {
            ...
            while(true) {
               ...
               do {
                  ...
                  entry = (Entry)it.next();
               } while(item.getStarLevel() < ((Integer)entry.getKey()).intValue());

               Iterator var18 = ((List)entry.getValue()).iterator();

               while(var18.hasNext()) {
                  ItemModify modify = (ItemModify)var18.next();
                  FinalModify fm = modify.createFinalModify();
                  list.add(fm);
               }
            }
         }

         if (item.getStones().size() > 0) {
            var8 = item.getStones().iterator();

            while(var8.hasNext()) {
               ItemStone stone = (ItemStone)var8.next();
               EquipStat equipStat = EquipStat.getEquipStat(stone.getEquipStatID());
               list.add(equipStat.createFinalModify());
            }
         }
