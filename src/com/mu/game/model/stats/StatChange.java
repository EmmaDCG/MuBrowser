...
import com.mu.io.game.packet.imp.player.AttributeChange;
import com.mu.io.game.packet.imp.player.DomineeringChange;
...
public class StatChange {
   ...
   public static boolean doSomethingWhenStatChange(Creature owner, HashMap statMap, boolean notice, boolean add, boolean fromEquip) {
      if (statMap == null) {
         return false;
      } else {
         boolean checkEquipment = false;
         ...
         while(it.hasNext()) {
            entry = (Entry)it.next();
            stat = (StatEnum)entry.getKey();
            int value = ((Integer)entry.getValue()).intValue();
            newValue = owner.getStatValue(stat);
            if (value != newValue && stat.getName().trim().length() > 0) {
               stats.add(stat);
            }
            ...
            case 8:
               if (newValue != value) {
                  ...
                  int hp = (int)(1.0D * (double)owner.getHp() / (double)value * (double)newValue);
                  owner.setHp(hp);
               }
               break;
            case 16:
               if (newValue != value) {
                  correctList.add(StatEnum.SD);
                  correctList.add(StatEnum.MAX_SD);
               }
               break;
            case 45:
               if (newValue != value) {
                  correctList.add(StatEnum.SPEED);
               }
               break;
            case 55:
               if (owner.getType() == 1) {
         ...
         } else if (!notice || !isSendStat(owner)) {
            sendStat = false;
         }
         ...
         if (sendStat) {
            if (owner.getType() == 1) {
               if (stats.size() > 0) {
                  if (domineeringUp > 0) {
                     DomineeringChange.sendToClient((Player)owner, domineeringUp);
                  }

                  PlayerAttributes.sendToClient((Player)owner, (List)stats);
                  HashMap changeMap = null;
                  Iterator var17 = stats.iterator();

                  while(var17.hasNext()) {
                     stat = (StatEnum)var17.next();
                     if (stat.isChangePrompt()) {
                        newValue = owner.getStatValue(stat);
                        newValue -= ((Integer)statMap.get(stat)).intValue();
                        if (stat.isPercent()) {
                           newValue /= 1000;
                        }
                        ...
                        if (newValue != 0) {
                           ...
                           changeMap.put(stat, newValue);
                        }
                     }
                  }

                  if (changeMap != null) {
                     AttributeChange.sendToClient((Player)owner, changeMap);
                     changeMap.clear();
                     entry = null;
                  }
               }
               ...
               if (checkEquipment) {
                  PlayerManager.dealWhenBasicPropertyChange((Player)owner, false);
               }
            }

            if (correctList.size() > 0) {
               CorrectAttributes.sendWhenChange(owner, (List)correctList);
            }
         }
   ...
   public static boolean isSendStat(Creature owner) {
      if (owner.isDestroy()) {
         return false;
      } else {
         return owner.getType() != 1 || !((Player)owner).isNew();
      }
   }

   public static void statChange(Creature creature, List modifies, boolean add) {
      HashMap changes = new HashMap();
      NumConversion conversion = NumConversion.getNumConversion(creature.getProfessionID());
      Iterator var6 = modifies.iterator();
      ...
      while(true) {
         ...
         do {
            do {
               ...
               fm = (FinalModify)var6.next();
               changes.put(fm.getStat(), Integer.valueOf(1));
            } while(conversion == null);
         } while(!conversion.getFirstAffectMaps().containsKey(fm.getStat()));

         Iterator var8 = ((SortedMap)conversion.getFirstAffectMaps().get(fm.getStat())).keySet().iterator();

         while(var8.hasNext()) {
            StatEnum stat = (StatEnum)var8.next();
            changes.put(stat, Integer.valueOf(1));
         }
      }
   }

   public static void addStat(Creature creature, int statID, List statList, boolean notice) {
      if (statList.size() >= 1) {
         HashMap statMap = getCompareMap(creature, statList);
         if (notice) {
            HashMap oldStatMap = creature.getProperty().getCompareMap(statID);
            if (oldStatMap != null) {
               Iterator var7 = oldStatMap.keySet().iterator();

               while(var7.hasNext()) {
                  StatEnum stat = (StatEnum)var7.next();
                  if (!statMap.containsKey(stat)) {
                     statMap.put(stat, (Integer)oldStatMap.get(stat));
                  }
               }
            }
         }

         creature.getProperty().addModifies(statID, statList);
         doSomethingWhenStatChange(creature, statMap, notice, true, StatIdCreator.getType(statID) == 7);
         statMap.clear();
         statMap = null;
      }
   }

   public static HashMap getCompareMap(Creature creature, List modifies) {
      ...
      while(true) {
         ...
         do {
            do {
               ...
               case 6:
                  map.put(StatEnum.STR, creature.getStatValue(StatEnum.STR));
                  map.put(StatEnum.CON, creature.getStatValue(StatEnum.CON));
                  map.put(StatEnum.INT, creature.getStatValue(StatEnum.INT));
                  map.put(StatEnum.DEX, creature.getStatValue(StatEnum.DEX));
                  break;
               case 26:
                  map.put(StatEnum.ATK_MIN, creature.getStatValue(StatEnum.ATK_MIN));
                  map.put(StatEnum.ATK_MAX, creature.getStatValue(StatEnum.ATK_MAX));
                  break;
               default:
                  map.put(modify.getStat(), creature.getStatValue(modify.getStat()));
               }
            } while(conversion == null);
         } while(!conversion.getFirstAffectMaps().containsKey(modify.getStat()));

         Iterator var7 = ((SortedMap)conversion.getFirstAffectMaps().get(modify.getStat())).keySet().iterator();

         while(var7.hasNext()) {
            StatEnum stat = (StatEnum)var7.next();
            map.put(stat, creature.getStatValue(stat));
         }
      }
   }

   public static void endStat(Creature creature, int statID, boolean notice) {
      HashMap statMap = creature.getProperty().endModify(statID, creature);
      if (statMap != null && statMap.size() > 0) {
         doSomethingWhenStatChange(creature, statMap, notice, false, StatIdCreator.getType(statID) == 7);
         statMap.clear();
      }
