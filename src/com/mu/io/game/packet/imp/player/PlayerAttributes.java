...
public class PlayerAttributes extends ReadAndWritePacket {
   // $FF: synthetic field
   private static int[] $SWITCH_TABLE$com$mu$game$model$stats$StatEnum;

   public PlayerAttributes() {
      super(10003, (byte[])null);
   }

   public PlayerAttributes(int code, byte[] readBuf) {
      super(code, readBuf);
   }
   ...
   public static void sendToClient(Player player, StatEnum stat) {
      List statList = new ArrayList();
      statList.add(stat);
      sendToClient(player, (List)statList);
      statList.clear();
   }

   public static void sendToClient(Player player, List stats) {
      PlayerAttributes pa = new PlayerAttributes();
      setStatDetail(player, stats, pa);
      player.writePacket(pa);
      pa.destroy();
      pa = null;
   }
   ...
   public void setData(Player player) {
      try {
         List stats = StatList2Client.getPlayerPanelStats();
         setStatDetail(player, stats, this);
   ...
   public static void setStatDetail(Player player, List stats, WriteOnlyPacket packet) {
      try {
         packet.writeShort(stats.size());
         Iterator var4 = stats.iterator();

         while(var4.hasNext()) {
            StatEnum stat = (StatEnum)var4.next();
            long value = getArributeValue(player, stat);
            writeStat(stat, value, packet);
         }
   ...
   public static void writeStat(StatEnum stat, long value, WriteOnlyPacket packet) {
      try {
         writeStat(stat, value, false, packet);
   ...
   public static void writeStat(StatEnum stat, long value, boolean isPrecent, WriteOnlyPacket packet) {
      try {
         packet.writeShort(stat.getStatId());
         packet.writeDouble((double)value);
         isPrecent = stat.isPercent() || isPrecent;
         packet.writeBoolean(isPrecent);
   ...
   public static long getArributeValue(Player player, StatEnum stat) {
      long value = 0L;

      try {
         switch($SWITCH_TABLE$com$mu$game$model$stats$StatEnum()[stat.ordinal()]) {
         case 23:
            value = (long)player.getAp();
            break;
         case 38:
            value = player.getCurretntExp();
            break;
         case 39:
            value = PlayerLevelData.getNeedExp(player.getLevel());
            break;
         case 41:
            value = (long)player.getMoney();
            break;
         case 43:
            value = (long)player.getIngot();
            break;
         case 44:
            value = (long)player.getBindIngot();
            break;
         case 47:
            value = (long)player.getPotential();
            break;
         case 48:
            value = (long)EvilManager.getShowEvil(player.getEvil());
            break;
         case 53:
            value = (long)player.getRedeemPoints();
            break;
         case 56:
            value = (long)(player.getStatValue(StatEnum.STR) + player.getStatValue(StatEnum.CON) + player.getStatValue(StatEnum.INT) + player.getStatValue(StatEnum.DEX) + player.getPotential());
            break;
         case 63:
            value = (long)player.getContribution();
            break;
         case 64:
            value = (long)player.getHisContribution();
            break;
         default:
            value = (long)player.getStatValue(stat);
         }
      } catch (Exception var5) {
         var5.printStackTrace();
      }
